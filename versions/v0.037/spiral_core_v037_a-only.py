# spiral_core v0.37-a-only (Non-narrative, Non-persona) <200 lines
...
class ObserveGate:
    # signature ignores dominant_count; payload keeps it. parents bind to conflict inputs set.
    ...
    def _make_top(self, rows, topn: int) -> Tuple[str,str,List[Tuple[str,int,str,int]]]:
        ...
        return ",".join(payload_parts), ",".join(sig_parts), kept

    def _select_observe_parents(self, h: History, win: int,
                               top_struct: List[Tuple[str,int,str,int]],
                               min_p: int=2, max_p: int=6) -> List[str]:
        # bind to inputs that match (k == dom_v) for at least one top_struct key
        recent_inputs = [e for e in h.events[-win:] if e.meta.get("kind") == "input"]
        want: List[str] = []
        for e in reversed(recent_inputs):  # newest first
            kv = parse_kv(e.payload)
            hit = False
            for (k, heat, dom_v, _) in top_struct:
                if heat > 0 and kv.get(k) == dom_v:
                    hit = True; break
            if hit:
                want.append(e.id)
                if len(want) >= max_p: break
        if len(want) >= min_p: return list(reversed(want))
        # fallback: pad with newest inputs (still within win)
    ...
    def maybe_observe(self, h: History, win: int, topn: int=3) -> Optional[Event]:
        ...
        if total <= 0:
            self.last_sig = None
            return None
        top_payload, top_sig, top_struct = self._make_top(rows, topn)
        sig = f"conflict_heat|win={win}|total={total}|struct_top={top_sig}"
        if self.inputs_since_emit < self.cooldown: return None
        if sig == self.last_sig: return None
...
class NoiseEngineAOnly:
    def __init__(self, seed: int=37, win: int=14, conflict_thr: int=2,
        ...
        self.cn_since = 10**9  # inputs since last conflict-noise
    ...
    def should_noise(self, h: History) -> Tuple[bool, str]:
        ...
        if c >= self.conflict_thr:
            if self.cn_since < self.cn_cool:
                return False, ""
            return True, f"conflict({c})"
        if c == 0 and h.events:
    ...
    def emit_noise(self, h: History, reason: str) -> Event:
        ...
        ev = h.append(parents, payload, meta={"kind":"noise","reason":reason})
...
def demo() -> None:
    h = History()
    eng = NoiseEngineAOnly(seed=37, win=14, conflict_thr=2, d_thr=14, n_thr=120,
