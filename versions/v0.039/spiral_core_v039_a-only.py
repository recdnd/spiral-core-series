# spiral_core v0.39-a-only (Non-narrative, Non-persona) <200 lines
...
class NoiseAOnly:
    def __init__(self, seed: int=39, win: int=14, conflict_thr: int=2, cooldown_inputs: int=2, backref_prob: float=0.15) -> None:
        random.seed(seed)
        ...
        self.since = 10**9
        self.last_sig: Optional[str] = None
        self.last_observe_parents: Optional[List[str]] = None  # NEW: bind conflict-noise parents to observe parents
    ...
    def should_conflict_noise(self, h: History) -> Tuple[bool, str, str]:
        total, sig, top_payload = self.conflict_meta(h)
        if total < self.thr:
            self.last_sig = None
            return False, "", ""
    ...
    def emit_conflict_noise(self, h: History, reason: str, top_payload: str) -> Event:
        # NEW: parents bound to last observe parents (conflict input set).
...
def frontier(h: History, last_inputs: int=3, anc_depth: int=4, topk: int=20) -> List[Event]:
    ...
    keep: Set[str] = set(keep_in)
    for e in h.events:
        if e.meta.get("kind") in ("noise","observe") and any(pid in keep_in for pid in e.parent_ids):
            keep.add(e.id)
...
def demo() -> None:
    ...
    eng = NoiseAOnly(seed=39, win=14, conflict_thr=2, cooldown_inputs=2, backref_prob=0.15)
    topics = ["x","y","z"]
    ...
    show("FRONTIER top20 (ranked by trace_score)", frontier(h, last_inputs=3, anc_depth=4, topk=20))
