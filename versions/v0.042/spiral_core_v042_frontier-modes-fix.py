# spiral_core_v042_frontier-modes-fix.py  (<200 lines)
...
class Clock:
    ...
    def tick(self, step: int = 1) -> int:
        # strictly monotonic timestamp even within same ms
        cur = now_ms()
...
def mk_input(clk: Clock, h: History, topic: str, label: str) -> Event:
    ts = clk.tick()
    pid = last_id(h); eid = rnd_id()
    return Event(ts, eid, [pid] if pid else [], {"kind":"input","topic":topic}, f"{label}; topic={topic}")

def mk_repair(clk: Clock, h: History, topic: str) -> Event:
    ts = clk.tick()
    pid = last_id(h); eid = rnd_id()
    return Event(ts, eid, [pid] if pid else [], {"kind":"input","topic":topic}, "repair: summarize; topic="+topic)

def mk_noise(clk: Clock, h: History, kind: str, parents: List[str], payload: str) -> Event:
    ts = clk.tick()
    return Event(ts, rnd_id(), parents, {"kind":"noise","noise_kind":kind}, payload)

def mk_observe(clk: Clock, h: History, parents: List[str], obs: str, payload: str) -> Event:
    ts = clk.tick()
    return Event(ts, rnd_id(), parents, {"kind":"observe","observe":obs}, payload)
...
def frontier(h: History, mode: str = "global", last_inputs: int = 4, last_observes: int = 2,
    ...
    if mode=="recent": evs = [e for e in evs if e.ts >= cutoff]
...
def main():
    ...
    WIN = 14
    COOLDOWN_CONFLICT_MS = 2  # keep your "cooldown=2" spirit, but now in ms on monotonic clock
    TOPICS = ["x","y","z"]
