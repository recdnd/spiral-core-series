# spiral_core v0.38-a-only (Non-narrative, Non-persona) <200 lines
...
def conflict_signature(total: int, rows: List[Tuple[str,int,str,int]], topn: int=3) -> Tuple[str,str,List[Tuple[str,int,str,int]]]:
    # payload keeps dom_count; signature omits it.
    payload_parts, sig_parts, kept = [], [], []
    for (k, heat, dom_v, dom_c) in rows[:topn]:
        if heat <= 0: break
        kept.append((k, heat, dom_v, dom_c))
        payload_parts.append(f"{k}:{heat}:{dom_v}:{dom_c}")
        sig_parts.append(f"{k}:{heat}:{dom_v}")
    payload_top = ",".join(payload_parts)
    sig_top = ",".join(sig_parts)
    sig = f"total={total}|top={sig_top}"
    return payload_top, sig, kept

class ObserveGate:
    def __init__(self, cooldown_inputs: int=2) -> None:
        self.last_sig: Optional[str] = None
        self.cooldown = max(0, cooldown_inputs)
        self.inputs_since_emit = 10**9
    def on_input(self) -> None: self.inputs_since_emit += 1

    def _select_observe_parents(self, h: History, win: int,
                               top_struct: List[Tuple[str,int,str,int]],
                               min_p: int=2, max_p: int=6) -> List[str]:
        recent_inputs = [e for e in h.events[-win:] if e.meta.get("kind") == "input"]
        want: List[str] = []
        for e in reversed(recent_inputs):
            kv = parse_kv(e.payload)
            hit = False
            for (k, heat, dom_v, _) in top_struct:
                if heat > 0 and kv.get(k) == dom_v: hit = True; break
            if hit:
                want.append(e.id)
    ...
    def maybe_observe(self, h: History, win: int, topn: int=3) -> Optional[Event]:
        ...
        full_sig = f"conflict_heat|win={win}|{sig}"
        if self.inputs_since_emit < self.cooldown: return None
        if full_sig == self.last_sig: return None
        self.last_sig = full_sig
        self.inputs_since_emit = 0
        parents = self._select_observe_parents(h, win, top_struct, min_p=2, max_p=6)
...
class NoiseEngineAOnly:
    def __init__(self, seed: int=38, win: int=14, conflict_thr: int=2,
                 d_thr: int=14, n_thr: int=120, backref_prob: float=0.15,
                 conflict_noise_cooldown: int=2) -> None:
        random.seed(seed)
        self.win, self.conflict_thr = win, conflict_thr
        self.d_thr, self.n_thr = d_thr, n_thr
        self.backref_prob = backref_prob
        self.cn_cool = max(0, conflict_noise_cooldown)
        self.cn_since = 10**9
        self.last_conflict_sig: Optional[str] = None
    ...
    def should_noise(self, h: History) -> Tuple[bool, str]:
        total, sig, _ = self.conflict_meta(h)
        if total >= self.conflict_thr:
            # signature gate + cooldown gate
            if self.cn_since < self.cn_cool: return False, ""
            if sig == self.last_conflict_sig: return False, ""
            self.last_conflict_sig = sig
            return True, f"conflict({total})"
        # reset conflict sig when calm
        if total == 0:
            self.last_conflict_sig = None
            if h.events:
    ...
    def emit_noise(self, h: History, reason: str) -> Event:
        ...
        if reason.startswith("conflict"):
            total, _, top_payload = self.conflict_meta(h)
...
def frontier_select(h: History, last_inputs: int=3, anc_depth: int=4, topk: int=20) -> List[Event]:
    ...
    def walk_in(eid: str, depth: int) -> None:
        ...
        e = h.by_id.get(eid)
        if not e: return
        if e.meta.get("kind") == "input": keep_in.add(eid)
        for pid in e.parent_ids: walk_in(pid, depth+1)
...
def demo() -> None:
    h = History()
    eng = NoiseEngineAOnly(seed=38, win=14, conflict_thr=2, d_thr=14, n_thr=120,
                           backref_prob=0.15, conflict_noise_cooldown=2)
