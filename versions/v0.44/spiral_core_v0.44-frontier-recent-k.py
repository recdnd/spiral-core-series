# spiral_core_v044_frontier-recent-k.py  (<200 lines)
...
def frontier(h, mode="global",
    ...
    if mode=="recent":
        # ✅ keep: roots + last K events + skeleton edges already in `evs`
        root_ids={r.id for r in roots}
        tail_ids={e.id for e in h.events[-recent_k:]}
        allow = root_ids | tail_ids | skeleton
        evs=[e for e in evs if e.id in allow]
...
def main():
    ...
    print_view("FRONTIER_RECENT_K top20 (ranked by trace_score)", fr_recent, h, n=20)
