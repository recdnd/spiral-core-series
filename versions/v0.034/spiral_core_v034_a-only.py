# spiral_core v0.34-a-only (Non-narrative, Non-persona) <200 lines
...
def observe_conflict_heat(h: History, win: int, topn: int=3) -> Optional[Event]:
    ...
    for (k, heat, dom_v, dom_c) in rows[:topn]:
        if heat <= 0: break
        parts.append(f"{k}:{heat}:{dom_v}:{dom_c}")
    payload = f"observe=conflict_heat; win={win}; total_heat={total}; top=" + ",".join(parts)
...
class NoiseEngineAOnly:
    def __init__(self, seed: int=29, win: int=14, conflict_thr: int=2,
    ...
    def should_noise(self, h: History) -> Tuple[bool, str]:
        # Priority: conflict -> (depth only if no conflict) -> count
...
def frontier_select(h: History, last_inputs: int=3, anc_depth: int=4, topk: int=20) -> List[Event]:
    # Frontier anchors: last N inputs + their input-ancestors; include observe if directly attached; include local noise
    ...
    for e in h.events:
        k = e.meta.get("kind")
        if k == "noise" and any(pid in keep_in for pid in e.parent_ids):
            keep_ids.add(e.id)
        if k == "observe" and any(pid in keep_in for pid in e.parent_ids):
            keep_ids.add(e.id)
...
def demo() -> None:
    h = History()
    eng = NoiseEngineAOnly(seed=31, win=14, conflict_thr=2, d_thr=14, n_thr=120, backref_prob=0.15)

    topics = ["x","y","z"]
    for i in range(28):
        t = random.choice(["x","y"]) if i % 4 in (0,1) else random.choice(topics)
        accept(h, eng, f"evt{i}:{random.randint(10**6,10**7-1)}; topic={t}")
        observe_conflict_heat(h, eng.win, topn=3)
    ...
    show(view_observe.name, view_observe.visible(h)[-8:])
